           ...
           scope.$watch(attrs.carousel, function(gists) {  
           	if(scope.gists.length > 0)
           	{
	            ...
	            for (var i = 0; i < gists.length; i++) {
	            	...
	            	var gistTitleLink = gists[i].poster_img || '/assets/img/posters/' + gists[i].title.replace('/', ' ') + '.jpg';
...
contentApp.controller('GistListCtrl', ['$scope', '$http', '$templateCache', 
	function($scope, $http, $templateCache) {
	  	$scope.url = 'http://graphgist-cms-api.herokuapp.com/api/v0/gists?api_key=special-key&neo4j=false';
	  	$scope.gists = [];
...
// 					var actorTitleLink = gist.actors[i].poster_img || '/assets/img/actors/' + gist.actors[i].name.replace('/', ' ') + '.jpg';
// 	                 html += '<div class="item">' +
// 						          '<div class="thumbnail">' +
           ...
           scope.$watch(attrs.carouselrelatedgists, function(gist) {  
           	if(scope.gist != undefined ? scope.gist.related != undefined ? scope.gist.related.length > 0 : false : false)
           	{
	            ...
	            for (var i = 0; i < gist.related.length; i++) {
					var relatedGistTitleLink = gist.related[i].related.poster_img || '/assets/img/posters/' + gist.related[i].related.title.replace('/', ' ') + '.jpg';
	                 html += '<div class="item">' +
						          '<div class="thumbnail">' +
...
contentApp.controller('GistItemCtrl', ['$scope', '$routeParams', '$http', '$templateCache',
  function($scope, $routeParams, $http, $templateCache) {
  		console.log('http://graphgist-cms-api.herokuapp.com/api/v0/gists/title/' + encodeURIComponent(decodeURI(decodeURI($routeParams.gistId))) + '?api_key=special-key&neo4j=false');
  		$scope.url = 'http://graphgist-cms-api.herokuapp.com/api/v0/gists/title/' + encodeURIComponent(decodeURI(decodeURI($routeParams.gistId))) + '?api_key=special-key&neo4j=false';
	  	var fetchGist = function()
	  	{
	  		$http({method: 'GET', url: $scope.url, cache: $templateCache}).
			    success(function(data, status, headers, config) {
			    	$scope.gist = data;
			    	$scope.gist.poster_img = $scope.gist.poster_img || '/assets/img/posters/' + $scope.gist.title.replace('/', ' ') + '.jpg';
			    	$scope.gist.poster_img = $scope.gist.poster_img.replace("w185", "w300");
			    }).
			    error(function(data, status, headers, config) {
			    // called asynchronously if an error occurs
			    // or server returns response with an error status.
           ...
           scope.$watch(attrs.carouselpeoplegists, function(people) {  
           	...
           	if(scope.people != undefined ? scope.people.gists != undefined ? scope.people.gists.length > 0 : false : false)
           	{
	            ...
	            for (var i = 0; i < people.gists.length; i++) {
	            	var relatedGistTitleLink = people.gists[i].poster_img || '/assets/img/posters/' + people.gists[i].title.replace('/', ' ') + '.jpg';
	                 html += '<div class="item">' +
						          '<div class="thumbnail">' +
           ...
           scope.$watch(attrs.carouselrelatedpeople, function(people) {  
           	if(scope.people != undefined ? scope.people.related != undefined ? scope.people.related.length > 0 : false : false)
           	{
	            ...
	            for (var i = 0; i < people.related.length; i++) {
					var actorTitleLink = people.related[i].related.poster_img || '/assets/img/actors/' + people.related[i].related.name.replace('/', ' ') + '.jpg';
	                 html += '<div class="item">' +
						          '<div class="thumbnail">' +
...
contentApp.controller('PeopleItemCtrl', ['$scope', '$routeParams', '$http', '$templateCache',
  function($scope, $routeParams, $http, $templateCache) {
  		console.log('http://graphgist-cms-api.herokuapp.com/api/v0/people/name/' + encodeURIComponent(decodeURI(decodeURI($routeParams.peopleId))) + '?api_key=special-key&neo4j=false');
  		$scope.url = 'http://graphgist-cms-api.herokuapp.com/api/v0/people/name/' + encodeURIComponent(decodeURI(decodeURI($routeParams.peopleId))) + '?api_key=special-key&neo4j=false';
	  	var fetchPeople = function()
	  	{
	  		$http({method: 'GET', url: $scope.url, cache: $templateCache}).
			    success(function(data, status, headers, config) {
			    	$scope.people = data;
			    	$scope.people.poster_img = $scope.people.poster_img || '/assets/img/actors/' + $scope.people.name.replace('/', ' ') + '.jpg';
			    }).
			    error(function(data, status, headers, config) {
			    // called asynchronously if an error occurs
			    // or server returns response with an error status.
